...
class DocumentModel extends Model{
     ...
     * @return array              文档列表
     */
    public function lists($category, $order = '`id` DESC', $status = 1, $field = true,$where=array()){
        $map = $this->listMap($category, $status,NULL,$where);
     ...
     * @return integer           总数
     */
    public function listCount($category, $status = 1,$where=array()){
        $map = $this->listMap($category, $status,NULL,$where);
     ...
     * @return array       详细数据
     */
    public function detail($id,$position=''){
        /* 获取基础数据 */
        ...
        if($info['model_id']==3){
        	$attribute=F('Arr/attribute','','./Data/');//获取attribute文件中的数组
        	if($attribute){
        		foreach($attribute as $key=>$val){
        			if($val['data'][$detail[$val['value']]]['name']){
        				$detail['attribute'][]=$val['data'][$detail[$val['value']]]['name'];
        			}
        		}
        	}
        }
        ...
        if($info['model_id']==3){
        	$detail['path'][]=array(
        			'src'=>U('index'),
        			'location'=>"首页【".$position."站】"
        	);
        	$detail['path'][]=array(
        			'src'=>U('houses/search_map'),
        			'location'=>"楼盘中心"
        	);
        	$detail['path'][]=array(
        			'src'=>U('houses/home',array('id'=>$info['id'])),
        			'location'=>$info['title'],
        	);
        }
    ...
    private function logic($model){
        ...
        return new $class($name,'zf_', $this->connection);
    }
     ...
     * @return array             查询条件
     */
    private function listMap($category, $status = 1, $pos = null,$where=array()){
        $map = $where;
        /* 设置状态 */
        $map['status']=$status;
        $map['pid']=0;
