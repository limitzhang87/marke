...
function getHouses($field = '')
{
    ...
    if(!$res){
        ...
        if($id == null){
            session(null);
            checkORredirect(is_login(),'Login/login');
            return ;
        }
        $houses = M('Document','zf_', 'mysql://zhang:123456@120.24.92.182:3306/zf')->field($field)->find($id);
        if(!$houses){
            $houses = M('document_download','zf_', 'mysql://zhang:123456@120.24.92.182:3306/zf')->field($field)->find($id);
        }
        $res =  $houses[$field];
        session($field,$res);
