...
class OwnerShipController extends HomeController {
    ...
    public function index($id = ''){
        if(IS_AJAX){
            ...
            imagettftext($img, $contentSize, 0, ($width-320)/2, $height-20, $fontcolor, $font, '技术支持：海南中房邦网络技术服务有限公司');
            ...
            if($a){
                ...
                $pic_data['houses'] =  session('db_name');
                if($pic_data['id'] = D('ownership_pic')->field('uid,path,created_time')->add($pic_data)){
                    $res['error'] = 0 ;
            ...
            $building = D('building')->field('id,b_name')->select();
            $unit = D('unit')->select();
            foreach ($unit as $vo) {
                $unit_ids[] = $vo['id'];
            }
            ...
            if($ownership_bg_img == ''){
                $ownership_man  = D('ownership_man')->find();
    ...
    public function lists()
    {
        ...
        $config['url'] =  $_SERVER['HTTP_HOST'] . U('Share/ownership_pic_show',array('houses'=>session('db_name'),'id'=>$id));
        ...
        $this->assign('houses',session('db_name'));
        $this->display();
    }
