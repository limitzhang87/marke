...
<script type="text/javascript" src="/marke/Public/static/uploadify/jquery.uploadify.min.js"></script>
...
<div class="tab-wrap">
    ...
    <!-- 表单 -->
    <form id="form" action="<?php echo U('set?model='.$model['id']);?>" method="post" class="form-horizontal">
        <!-- 基础文档模型 -->
		<?php $_result=parse_config_attr($model['field_group']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><div id="tab<?php echo ($key); ?>" class="tab-pane <?php if(($key) == "1"): ?>in<?php endif; ?> tab<?php echo ($key); ?>">
            <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['is_show'] == 1 || $field['is_show'] == 3): ?><div class="form-item cf">
                    ...
                    <div class="controls">
                        <?php switch($field["type"]): case "houses_dialog": echo hook('Houses_select', array('category_id'=>$data['category_id'],'houses_id'=>$data['houses_id'])); break;?>
                            ...
                            <?php case "picture": ?><div class="controls">
									...
									<?php if(!empty($data[$field['name']])): ?><div class="upload-pre-item"><img src="/marke<?php echo (get_cover($data[$field['name']],'path')); ?>"/></div><?php endif; ?>
									</div>
								</div>
								...
								$("#upload_picture_<?php echo ($field["name"]); ?>").uploadify({
							        "height"          : 30,
							        "swf"             : "/marke/Public/static/uploadify/uploadify.swf",
...
<?php if(C('COLOR_STYLE')=='blue_color') echo '<link href="/marke/Public/static/datetimepicker/css/datetimepicker_blue.css" rel="stylesheet" type="text/css">'; ?>
