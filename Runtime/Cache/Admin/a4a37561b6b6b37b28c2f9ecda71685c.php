...
<body>
    ...
    <div class="tab-wrap">
        ...
        <div class="tab-content">
            <!-- 表单 -->
            <form id="form" action="<?php echo U('menu_auth',array('m_id'=>$m_id));?>" method="post" class="form-horizontal">
                <!-- 基础文档模型 -->
                <div id="tab" class="tab-pane in">
                    <label class="item-label">请输入添加权限并选择是否开启</label>
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="controls" style="width: 400px;display: -webkit-box;">
                            <input type="hidden" name="id[]" value="<?php echo ($vo["id"]); ?>">
                            <input type="text" class="text input-large" name="title[]" value="<?php echo ($vo["title"]); ?>" style="width: 150px;">
<!--                             <div style="margin: 0 10px;">
                                <select name="is_auth[]">
                                    <option value="1" <?php if(($vo['is_auth']) == "1"): ?>selected<?php endif; ?>>开启</option>
                                    <option value="0" <?php if(($vo['is_auth']) != "1"): ?>selected<?php endif; ?>>关闭</option>
                                </select>
                            </div> -->
                            <div class="btn submit-btn confirm" style="margin: 0 10px; padding: 5px 10px;" onclick="del(this,'<?php echo ($vo["id"]); ?>')">删除</div>
    ...
    <?php if(C('COLOR_STYLE')=='blue_color') echo '
        <link href="/marke/Public/static/datetimepicker/css/datetimepicker_blue.css" rel="stylesheet" type="text/css">'; ?>
    ...
    function del(i, id = 0) {
        if (id != 0) {
            if (confirm('是否删除？')) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/marke/index.php?s=/Admin/MenuList/del_auth",
                    data: "id="+id,
                    error:function(){
                        alert('删除失败');
                    },
