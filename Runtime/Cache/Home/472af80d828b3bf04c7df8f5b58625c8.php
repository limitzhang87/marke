...
<body>
        ...
        <div class="info">
            <section>
                <ul>
                    ...
                    <li>下单时间：<?php echo (date("Y-m-d H:i:s",$_info["first_time"])); ?></li>
                    <li>是否本省：<?php echo (getyn($_info["in_province"])); ?></li>
                    <li>分销客户：<?php echo ($_info["nick_name"]); ?></li>
                </ul>
            ...
            <section>
                <ul>
                    <li>预约时间：<?php echo (date("Y-m-d H:i:s",$_info["order_time"])); ?></li>
                    <li>带看人员：<?php echo ($_info["rec_name"]); ?></li>
                    <li>调度人员：<?php echo ($_info["dis_name"]); ?></li>
                    <li>车辆信息：<?php echo ($_info["care"]); ?></li>
                    <li>接车地点：<?php echo ($_info["meet_address"]); ?></li>
                </ul>
            </section>
            <li class="line"></li>
            <section>
                <!-- 根据不同权限设置不同功能 -->
                <?php if(is_auth(AUTH_BBORDER_MANAGE)): ?><div class="dispatch">
            ...
            <li class="line"></li>
            <?php if($_info['rec_uid'] == is_login()): ?><section>
                <?php if($_info['over'] != 1 && $_info['check_out'] != 1): ?><button class="status_success" onclick="add_log('电话回访')">电话回访</button>
    ...
    <?php if(is_auth(AUTH_BBORDER_MANAGE)): ?><div class="fixed_add" style="background-color: <?php echo ($bg_color); ?>; padding: 6px;"  onclick="redirect('<?php echo U('edit',array('id'=>$_info['id']));?>')">
            <img src="/marke/Public/Mobile/images/icon/edit.png">
        </div><?php endif; ?>
    <?php if(is_auth(AUTH_TRACK_MANAGE)): if($_info["status"] == 1): ?><center style="margin-top: 10px;" >
...
//})();
    ...
    <script type="text/javascript">
        ...
        function add_log(type) {
            ...
            $.ajax({
                url:'/marke/index.php?s=/Home/BBorder/add_log',
                type:'post',
    ...
    <script type="text/javascript">
        ...
        function chagneRec() {
            if(!confirm('是否确认调度')){
                ...
                $.ajax({
                    url:'/marke/index.php?s=/Home/BBorder/changeRec',
