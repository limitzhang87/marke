...
<body>
    ...
    <center style=" overflow: hidden">
        <?php if(is_auth(AUTH_BBORDER_ADD)): ?><div class="block" onclick="redirect('<?php echo U('add');?>')">
            <div class="icon"></div>
            <div class="blick_title">客户备案添加</div>
        </div><?php endif; ?>
        <?php if(is_auth(AUTH_BBORDER_MANAGE)): ?><div class="block" onclick="redirect('<?php echo U('over_time');?>')">
            <div class="icon"></div>
            <div class="blick_title">修改保护期时间</div>
        </div><?php endif; ?>
        <?php if(is_array($button)): $i = 0; $__LIST__ = $button;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$btn): $mod = ($i % 2 );++$i;?><div class="block" onclick="redirect('<?php echo U('index',array('type'=>$btn['class']));?>')">
                <div class="icon"></div>
                <div class="blick_title"><?php echo ($btn["title"]); ?></div>
            </div><?php endforeach; endif; else: echo "" ;endif; ?>


        <div class="block" onclick="redirect('<?php echo U('common');?>')">
            <div class="icon"></div>
            <div class="blick_title">统计表</div>
        </div>
        <!-- 同解决子标签使用float而父标签无法自适应的问题 -->
       <!--  <div style="clear: both;"></div> -->
    </center>
    <?php if(is_auth(AUTH_BBORDER_ADD)): ?><div class="fixed_add" style="background-color: <?php echo ($bg_color); ?>; padding: 6px;" onclick="redirect('<?php echo U('add');?>')">
