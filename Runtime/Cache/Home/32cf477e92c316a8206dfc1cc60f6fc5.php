...
<body>
    ...
    <form action="" method="POST">
        ...
        <center style="color: #999999" >
            每个菜单前面输入数字进行菜单显示排序(不填的默认在最面前)！
        </center>
        <?php if(is_array($menuList)): $i = 0; $__LIST__ = $menuList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><div class="form_control_div" style="font-size: 1.2rem;height: 2rem;line-height: 2.5rem;">
                <input type="number" name="s_<?php echo ($menu["id"]); ?>" style="width: 2.5rem;height: 1.5rem;text-align: center;" value="<?php echo ($menu["sort"]); ?>">
                <input type="checkbox" name="group_menu[]" value="<?php echo ($menu["id"]); ?>" <?php if(in_array($menu['id'],$gmids)): ?>checked="checked"<?php endif; ?>><?php echo ($menu["title"]); ?>
