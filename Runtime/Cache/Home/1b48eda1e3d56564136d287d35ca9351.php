...
<body>
    ...
    <form action="" method="POST">
        ...
        <div style="width: 100%;height:2rem; background-color: #EBEBEB;">
            <div style="line-height: 2rem;padding-left: 10px;">添加权限(<?php echo count($_list);?>)(添加菜单没有添加权限，同样不能访问)</div>
        </div>
        <div>
        <?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="item_s">
                <div class="item_info" style="width: 100%;">
                    <span class="item_menu" style="background: url('/marke<?php echo get_cover($vo['icon']);?>') no-repeat;"><?php echo ($vo["title"]); ?></span>
                    <span class="menu_auth">
                        <?php if(is_array($vo["auth"])): $i = 0; $__LIST__ = $vo["auth"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$auth): $mod = ($i % 2 );++$i;?><div  style="display: -webkit-box;margin: 0 3px;height: 100%;;">
                                <input type="checkbox" name="gm_<?php echo ($vo["gm_id"]); ?>[]" value="<?php echo ($auth["id"]); ?>" <?php if(in_array($auth['id'],$vo['auth_ids'])): ?>checked<?php endif; ?>>
                                <span><?php echo ($auth["title"]); ?></span>
        ...
        <center>
            <button class="button btn_submit" style="margin: 10px auto;"  type="submit">
               提交
            </button>
        </center>
    </form>
