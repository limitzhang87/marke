...
<body>
    ...
    <form method="POST">
        <?php if(is_array($group)): $i = 0; $__LIST__ = $group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="item_s">
            <div class="item_info">
                <input type="radio" name="g_id" <?php if($vo['id'] == $groupuser['g_id']): ?>checked="checked"<?php endif; ?> value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?>
            </div>
        </div><?php endforeach; endif; else: echo "" ;endif; ?>
        <center style="display: -webkit-box;width: 100%:">
            <input type="hidden" name="gu_id" value="<?php echo ($groupuser["id"]); ?>">
            <input type="hidden" name="uid" value="<?php echo ($groupuser["uid"]); ?>">
