...
<body>
    ...
    <form action="" method="POST">
        ...
        <?php if(is_array($bottomList)): $i = 0; $__LIST__ = $bottomList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bottom): $mod = ($i % 2 );++$i;?><div class="form_control_div" style="font-size: 1.2rem;height: 2rem;line-height: 2.5rem;">
                <input type="number" name="s_<?php echo ($bottom['m_id']); ?>" style="width: 2.5rem;height: 1.5rem;text-align: center;" value="<?php echo ($bottom["sort"]); ?>">
                <input type="checkbox" name="group_menu_b[]" value="<?php echo ($bottom['m_id']); ?>" <?php if(in_array($bottom['m_id'],$gmm_ids)): ?>checked="checked"<?php endif; ?>><?php echo ($bottom["title"]); ?>
            </div><?php endforeach; endif; else: echo "" ;endif; ?>
        </div>
        <center>
            <input type="hidden" name="id" value="<?php echo ($_id); ?>">
            <button type="submit" class="button btn_submit" >提交</button>
        </center>
    </form>
