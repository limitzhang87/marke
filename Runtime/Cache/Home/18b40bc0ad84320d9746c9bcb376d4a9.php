...
<body>
   ...
   <div class="tab-wrap">
    <div class="tab-nav">
        <?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><div data-tab="tab<?php echo ($key); ?>" <?php if(($key) == "1"): ?>class="current"<?php endif; ?>><?php echo ($group); ?></div><?php endforeach; endif; else: echo "" ;endif; ?>
    </div>
    <br>
    <div class="tab-contentform-horizontal">
    <!-- 表单 -->
        <!-- 基础文档模型 -->
        <?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><div id="tab<?php echo ($key); ?>" class="tab-pane <?php if(($key) == "1"): ?>in<?php endif; ?> tab<?php echo ($key); ?>">
            <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['is_show'] == 1 || $field['is_show'] == 3): ?><div class="form-item cf">
                    <div class="item-label"><?php echo ($field['title']); ?><span class="check-tips"><?php if(!empty($field['remark'])): ?>（<?php echo ($field['remark']); ?>）<?php endif; ?>:</span></div>
                    <div class="controls">
                        <?php switch($field["type"]): case "num": ?><!-- <input type="text" class="text input-medium" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"> -->
                                <span><?php echo ($data[$field['name']]); ?></span><?php break;?>
                            ...
                            <?php case "datetime": ?><!-- <input type="text" name="<?php echo ($field["name"]); ?>" class="text input-large time" <?php if($data[$field['name']] > 0): ?>value="<?php echo (date('Y-m-d',$data[$field['name']])); ?>"<?php endif; ?> placeholder="请选择时间" /> -->
                                <span><?php if($data[$field['name']] > 0): echo (date('Y年m月d日',$data[$field['name']])); endif; ?></span><?php break;?>
                            <?php case "bool": ?><select disabled name="<?php echo ($field["name"]); ?>">
                            ...
                            <?php case "select": ?><select disabled name="<?php echo ($field["name"]); ?>">
                                    <?php $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if(($data[$field['name']]) == $key): ?>selected<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                </select><?php break;?>
                            <?php case "radio": $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label class="radio">
                                    <input disabled type="radio" value="<?php echo ($key); ?>" name="<?php echo ($field["name"]); ?>" <?php if(($data[$field['name']]) == $key): ?>checked="checked"<?php endif; ?>><?php echo ($vo); ?>
                            ...
                            <?php case "checkbox": $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label class="checkbox">
                                    <input type="checkbox" disabled value="<?php echo ($key); ?>" name="<?php echo ($field["name"]); ?>[]" <?php if(in_array(($key), is_array($data[$field['name']])?$data[$field['name']]:explode(',',$data[$field['name']]))): ?>checked="checked"<?php endif; ?>><?php echo ($vo); ?>
                                    </label><?php endforeach; endif; else: echo "" ;endif; break;?>
                            <?php default: ?>
...
<!--         <center>
        ...
        <?php if(is_auth(AUTH_CUSTMER_ADD) || is_auth(AUTH_CUSTMER__MANAGE)): ?><div class="fixed_add" style="background-color: <?php echo ($bg_color); ?>; padding: 6px;"  onclick="redirect('<?php echo U('edit',array('id'=>$data['id']));?>')">
