...
<body>
    ...
    <div id="content" style="width: 90%;margin: 5px auto;">
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div id="p_<?php echo ($vo["id"]); ?>">
                <div class="close_red right" onclick="delImg('<?php echo ($vo["id"]); ?>')">X</div>
                <img src="/marke<?php echo ($vo["path"]); ?>" data-url="<?php echo ($vo["path"]); ?>">
     ...
     <?php if(is_auth(AUTH_HOUSESIMG_UPLOAD)): ?><div class="fixed_add" style="background-color: <?php echo ($bg_color); ?>; padding: 6px;" onclick="redirect('<?php echo U('HousesImg/add');?>')">
