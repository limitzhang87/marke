...
<body>
    ...
    <div>
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><div class="info" onclick="toRoom('<?php echo ($info["id"]); ?>')">
                <div class="l_info">
                    <div class="info_title">合同地址：</div>
                    <div><?php echo getHouses('title'); echo ($info["b_name"]); ?>(栋)<?php echo ($info["u_name"]); ?>(单元)<?php echo ($info["room_number"]); ?></div>
                </div>
                <div class="l_info">
                    <div class="info_title">收藏人数：</div>
                    <div><?php echo getCollectNum($info['id']);?></div>
                </div>
                <div class="s_info">
                    <div class="info_title">房号：</div>
                    <div><?php echo ($info["u_name"]); ?> - <?php echo ($info["room_number"]); ?></div>
                </div>
                <div class="s_info">
                    <div class="info_title">状态：</div>
                    <div><?php if($info['status'] == '0' ): ?>未售<?php else: ?>已售<?php endif; ?></div>
                </div>
                <div class="s_info">
                    <div class="info_title">面积：</div>
                    <div><?php echo ($info["area"]); ?> m²</div>
                </div>
                <div class="s_info">
                    <div class="info_title">朝向：</div>
                    <div><?php echo ($info["orientation"]); ?></div>
                </div>
                <div class="l_info">
                    <div class="info_title">间隔：</div>
                    <div><?php echo ($info["apartment"]); ?>室<?php echo ($info["hall"]); ?>厅<?php echo ($info["kitchen"]); ?>厨<?php echo ($info["toilet"]); ?>卫</div>
                </div>
