...
function getBuilding(){
    $.ajax({
        url:'./index.php?s=/Raise/Room/getBuilding',
        type:'POST',
        data:{'status':status},
        beforeSend:function(){
...
function getUnit(){
    $.ajax({
        url:'./index.php?s=/Raise/Room/getUnit',
        type:'POST',
        data:{'b_id' : b_id,'status':status},
        beforeSend:function(){
...
function getRoom(){
    $.ajax({
        url:'./index.php?s=/Raise/Room/getRoom',
        type:'POST',
        data: {'u_id':u_id,'status':status},
        beforeSend:function(){
            $('#room_list').html('<li class="load_icon_3"></li>');
        },
        error:function(msg){
            $('#room_list').html('<div style="width: 100%;text-align: center;font-size: 1.5rem;margin: 10px 0;">暂无房号信息！</div>');
        },
    ...
    for(var li in list){
        if(type == 1){
            ...
            $(list[li]).each(function(i,item){
                var status = '';
                if(item.status == 1){
                    html += '<div onclick="toRoom(\''+item.id+'\');"><div class="room_number lock_icon '+item.statusCSS+ '">'+item.room_number+'</div>';
                    html += '<div class="room_nick_name" >'+item.nick_name+'</div>';
                }else if(item.status == 9){
                    html += '<div onclick="toRoom(\''+item.id+'\');"><div class="room_number submit_icon '+item.statusCSS+ '">'+item.room_number+'</div>';
                    html += '<div class="room_nick_name_s" >'+item.nick_name+'</div>';
                }else if(item.status == 2){
                    html += '<div onclick="toRoom(\''+item.id+'\');"><div class="room_number  submit_icon '+item.statusCSS+ '">'+item.room_number+'</div>';
                    html += '<div class="room_nick_name_s" >'+item.nick_name+'</div>';
                }else{
                    html += '<div style="overflow-x: hidden;"  onclick="toRoom(\''+item.id+'\');"><div class="room_number '+item.statusCSS+ '">'+item.room_number+'</div>';
                }
                html += '</div>';
            });
        html +=   '</div>'+
