...
function getList(url){
	if(url == '' || url == undefined){
		url =ThinkPHP.APP + '/Home/BBorder/getList';
	}
	if(time == "*"){
	...
	$.ajax({
		...
		'field':field,'name':name,'time':time,'status':status,'time_style':time_style},
		beforeSend:function(){
    ...
    if(info._data != '' && info._data != null){
    	$.each(info._data,function(i,item){
    		if(i >= info._data.length-1){
    			...
    			body += '<tr onclick="redirect(\''+ThinkPHP.APP+'/Home/BBorder/detail/id/'+item.id+'\')">';
    			body += '<td>'+((page-1)*10 +1+i)+'</td>';
    			//body += '<td style="color:blue;" >跟踪</td>';
    			$.each(info._grids,function(ii,itemG){
	    			body += '<td>'+get_list_field(item,itemG)+'</td>'
	    		});
	    		//body += '<td style="color:blue;" onclick="redirect(\''+ThinkPHP.APP+'/Home/BBorder/edit/id/'+item.id+'\')">编辑</td>';
    		}
	    	body += '</tr>';
	    });
    }
...
function downloadExcel(title) {
    ...
    myform.attr('action',ThinkPHP.APP + '/Home/BBorder/downloadExcel');

	var data = {'uid':uid,'search_key':search_key,'kw':kw,'order':order,'where':where,'page':page,'fields':fields,'unfields':unfields,
		'field':field,'name':name,'time':time,'status':status,'time_style':time_style};
...
function getListByPage(p){
	page = p;
	var url = ThinkPHP.APP + '/Home/BBorder/getList/p/' + p;
	getList(url);
}
...
function getStastics(){
	$.ajax({
		url:ThinkPHP.APP + '/Home/BBorder/getStastics',
		type:'POST',
