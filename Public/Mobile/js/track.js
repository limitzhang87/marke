...
function getList(url){
	if(url == '' || url == undefined){
		url =ThinkPHP.APP + '/Home/Track/getList';
	}
	if(time == "*"){
	...
	$.ajax({
		url:url,
		type:'POST',
		data:{'uid':uid, 'search_key':search_key,'kw':kw,'order':order,'where':where,'page':page,'fields':fields,'unfields':unfields,
		'field':field,'name':name,'time':time,'status':status},
		beforeSend:function(){
    ...
    if(info._data != '' && info._data != null){
    	$.each(info._data,function(i,item){
    		if(i >= info._data.length-1){
    			...
    			body += '<tr onclick="redirect(\''+ThinkPHP.APP+'/Home/Track/detail/id/'+item.id+'\')">';
    			body += '<td>'+((page-1)*10 +1+i)+'</td>';
    			//body += '<td style="color:blue;" >跟踪</td>';
    			$.each(info._grids,function(ii,itemG){
	    			body += '<td>'+get_list_field(item,itemG)+'</td>'
	    		});
	    		//body += '<td style="color:blue;" onclick="redirect(\''+ThinkPHP.APP+'/Home/Track/edit/id/'+item.id+'\')">编辑</td>';
    		}
	    	body += '</tr>';
	    });
    }
...
function downloadExcel(title) {
    ...
    myform.attr('action',ThinkPHP.APP + '/Home/Track/downloadExcel');

	var data = {'title':title,'uid':uid, 'search_key':search_key,'kw':kw,'order':order,'where':where,'page':page,'fields':fields,
	'unfields':unfields,'field':field,'name':name,'time':time,'status':status};
...
function getListByPage(p){
	page = p;
	var url = ThinkPHP.APP + '/Home/Track/getList/p/' + p;
	getList(url);
}
...
function getStastics(){
	$.ajax({
		url:ThinkPHP.APP + '/Home/Track/getStastics',
		type:'POST',
...
function getStar(v) {
	var html = '<div>';
	for (var i = 1; i <= parseInt(v); i++) {
		html += '<img style="width:auto;" src="'+ThinkPHP.ROOT+'/Public/static/raty-2.5.2/lib/img/star-on.png">';
	}
	for (var i = v; i <5; i++) {
		html += '<img style="width:auto;" src="'+ThinkPHP.ROOT+'/Public/static/raty-2.5.2/lib/img/star-off.png">';
	}
	html += '</div>'
	return html ;
}
