...
$(function () {
    ...
    var c = document.cookie.indexOf("onethink_home_visit_key"); 
    
    if (is_login()||c!=-1) {  //刷新信息
    ...
    $('[ucard]').qtip({ // Grab some elements to apply the tooltip to
        ...
        content: {
            text: function (event, api) {
                ...
                $.get(U('Public/getProfile'), {uid: uid}, function (userProfile) {
                    var follow = '';
                    if ((MID != uid) && (MID != 0)) {
                    ...
                    var html = '<div class="row" style="width: 350px;width: 350px;font-size: 13px;line-height: 23px;">' +
                        ...
                        '<img class="img-responsive" src="' + window.Think.ROOT + '/Public/uc/Core/images/qtip_bg.png">' +
                        '</div>' +
...
function checkMessage() {
    $.get(U('message/getInformation'), {}, function (msg) {
        if (msg.messages) {
        ...
        if (msg.new_talk_messages) {
            ...
            $.each(msg.new_talk_messages, function (index, talk_message) {
                    if (message_box_showing(talk_message)) {
                        ...
                        $.get(U('Session/getSession'), {id: talk_message.talk_id}, function () {
...
function readMessage(obj, message_id) {
    ...
    $.post(U('Public/readMessage'), {message_id: message_id}, function (msg) {
        if (msg.status) {
            location.href = url;
        }
    }, 'json');
...
function setAllReaded() {
    $.post(U('Public/setAllMessageReaded'), function () {
        $hint_count.text(0);
...
var talker = {
    ...
    start_talk: function (uid,flag) {
        if (confirm('确定要和该用户发起聊天？')) {
        	if(flag==''){
        		...
        		$.post(U('Session/createTalk'), {uids: uid}, function (msg) {
                    if (msg.status) {
                        toast.success('聊天发起成功。', '聊天助手');
                        $('#friend_panel_main').toggle();
                        $('#session_panel_main').toggle();
    ...
    fetch_message_tpl: function (message, mid) {
        var tpl_right = '<div class="row talk_right">' +
            ...
            '<div class="row">' +
            '<div class="col-md-10 bubble_outter">' +
            '<h3>我&nbsp;</h3><i class="bubble_sharp"></i>' +
            '<div class="talk_bubble">{content}' +
            '</div>' +
            '</div>' +
            '</div> </div>';
        ...
        var tpl_left = '<div class="row">' +
            '<div class="time"><span class="timespan">{ctime}</span></div>' +
            '<div class="row">' +
            '<div class="col-md-10 bubble_outter chat_bubble">' +
    ...
    exit: function (id) {
        if (confirm('确定退出该聊天？退出后无法再主动加入。')) {
            ...
            $.post(U('Message/doDeleteTalk'), {talk_id: id}, function (msg) {
                if (msg.status) {
                    $('#chat_box').hide();
                    $('#chat_li_' + id).remove();
    ...
    post_message: function () {
        ...
        $.post(U("Message/postMessage"), {talk_id: $('#chat_id').val(), content: $('#chat_content').val()}, function (msg) {
            if(!msg.status){
     ...
     */
    open: function (id) {
        $.get(U('Session/getSession'), {id: id}, function (data) {
            talker.clear_box();
            ...
            $('#chat_box').show();
            $('#right_panel_main').css('display','none');
    ...
    prepend_session: function (data) {
    	if(data.ico==''||data.ico==null){
    		 data.ico='Addons/Avatar/default_64_64.jpg';//添加默认头像
    	}
...
//          "talker.exit(" + data.id + ")" +
//          '"><i style="color: red" title="退出聊天" class="fa fa-power-off"></i></a></div></div></div></li>';
            data.title + '</a></div></div></div></li>';
...
var getFace = function (obj) {
	var i=0;
    $.post(U('Index/getSmile'), {}, function (data) {
        var _imgHtml = '';
        for (var k in data) {
