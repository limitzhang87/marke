...
class Collection {
    ...
    public function _initialize() {
    	parent::_initialize();
    	import('Collection.ORG.phpQuery','./Addons');
    ...
    public static function get_content($url, $config, $page = 0) {
       ...
       // $html = self::get_html($url, $config);
        ...
        $reg = array(
        		"title"=>array($config['title_rule'],((stristr($config['title_html_rule'],'纯文本'))?'text':'html')),
        		"content"=>array($config['content_rule'],((stristr($config['content_html_rule'],'纯文本'))?'text':'html')),
        		"author"=>array($config['author_rule'],((stristr($config['author_html_rule'],'纯文本'))?'text':'html')),
        		"time"=>array($config['time_rule'],((stristr($config['time_html_rule'],'纯文本'))?'text':'html')),
        		"comeform"=>array($config['comeform_rule'],((stristr($config['comeform_html_rule'],'纯文本'))?'text':'html')),
        );
        //对自定义数据进行采集
        ...
        if ($config['customize_config'] && is_array($config['customize_config'])) {
        	foreach ($config['customize_config'] as $k => $v) {
        		...
        		$reg[$v['en_name']]=array($v['rule'],((stristr($v['html_rule'],'纯文本'))?'text':'html'));
        	}
        }
        $rs=  QueryList::Query($url,$reg,'','curl','UTF-8');
      ...
      //  PRINT_R($data);EXIT;
        if ($data) {
            ...
            if (empty($page)) {
                ...
                if ($config['title_rule']) {
                  //  $title_rule = self::replace_sg($config['title_rule']);
                    $data['title'] = self::replace_item($data['title'], $config['title_html_rule']);
                }

                //获取作者
                if ($config['author_rule']) {
                   // $author_rule = self::replace_sg($config['author_rule']);
                    $data['author'] = self::replace_item($data['author'], $config['author_html_rule']);
                }

                //获取来源
                if ($config['comeform_rule']) {
                   // $comeform_rule = self::replace_sg($config['comeform_rule']);
                    $data['comeform'] = self::replace_item($data['comeform'], $config['comeform_html_rule']);
                }

                //获取时间
                if ($config['time_rule']) {
                  //  $time_rule = self::replace_sg($config['time_rule']);
                    $data['time'] = strtotime(self::replace_item($data['time'], $config['time_html_rule']));
                }
                ...
                if ($config['customize_config'] && is_array($config['customize_config'])) {
                    foreach ($config['customize_config'] as $k => $v) {
                        ...
                        $data[$v['en_name']] = self::replace_item($data[$v['en_name']], $v['html_rule']);
                    }
                }
            }
            ...
            if ($config['content_rule']) {
                ...
                $data['content'] = self::replace_item($data['content'], $config['content_html_rule']);
                
                $data['content']=replace_txt($data['content'],'　　');
                $data['content'] = preg_replace("/<style[^>]*?>.+<\/style>/is", "",  $data['content']);// -----删除<style></style>和中间的部分
                $data['content'] = preg_replace("/<script[^>]*?>.+<\/script>/is", "",  $data['content']);// -----删除<style></style>和中间的部分
                
                $data['content']='<div class="kindeditor_html_div">'.$data['content'].'</div>';
            ...
            if ($page == 0) {
                ...
                if (empty($page) && !empty($data['content']) && $config['down_attachment'] == 1) {
                    //下载远程图片到本地
                	import('Collection.ORG.Attachment','./Addons');
                	$options=array(
                        "module" => "Collection",
                        "catid" => 0,
                        "isadmin" => 1,
                    );
                	$connect = new Attachment($options);
                	$Attachment = $connect->connect('Attachment', $options);
//                 	$Attachment =\Libs\System\Service::getInstance('Attachment', array(
//                         "module" => "Collection",
//                         "catid" => 0,
//                         "isadmin" => 1,
//                     ));
    ...
    public static function url_list(&$config, $num = '') {
        $url = array();
        if(!empty($config['urlpage_first'])){
        	$url[] =$config['urlpage_first'];
        }
...
//         }
    ...
    protected static function replace_item($html, $config) {
        if (empty($config)||(stristr($config,'纯文本'))) {//如果为 或者 纯文本 直接跳过
        	$config=str_replace('纯文本','',$config);
          //  return $html;
        }
        ...
        foreach ($config as $k => $v) {
            ...
            if (substr($v, 0, 4) == 'fun=') {
             ...
             //   $html = preg_replace('/<div [^>]*class="foot"[^>]*>(.*?)<\/div>/i', $replace, $html);
//                 if (!empty($v)&&$v!=' ') {
//                 //	$par="###,array('xx'=>array('.foot','html')),''";
//                 	$args[0]='.foot';
//                 	$args[1]='';
//                 //	$par = explode(",", $args[1]);
//                 //	$par[]=$html;
...
//                 	$html=replace_html($html,$args);
//                 //	$html = call_user_func_array('replace_html', $par);
          
// //                 	$rs=  QueryList::Query($html,array("xx"=>array('.foot','html')));
// //                 	$rs_data = $rs->jsonArr;
// //                 	$data=$rs_data[0];
