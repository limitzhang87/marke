...
class QueryList
{
    ...
    private function _getList()
    {
    	if( preg_match('/<meta[^>]+charset=/i', $this->html)) {
    		...
    		$charset = mb_check_encoding($this->html, 'utf-8') ? 'utf-8' : 'gbk';
    	
    		$this->html = sprintf('<meta http-equiv="Content-Type" content="text/html; charset=%s">%s', $charset, $this->html);

    	}
    
      if( $this->htmlEncoding=='CP936'){
      	$charset='UTF-8';//设置编码
      	$hobj = phpQuery::newDocumentHTML($this->html,$charset);
      }else{
        ...
        if (!empty($this->regRange)) {
            ...
            while (list($key, $reg_value) = each($this->regArr)) {
                if($key=='callback'||empty($reg_value[0]))continue;//如果规则为空或者是回调函数
                if( $this->htmlEncoding=='CP936'){
                	$charset='UTF-8';//设置编码
                	$hobj = phpQuery::newDocumentHTML($this->html,$charset);
                }else{
        ...
        if ($this->outputEncoding) {
        	//print_r( $this->htmlEncoding );exit;
        	if( $this->htmlEncoding=='CP936'){
    ...
    {
    	if(preg_match('/http:\/\/[\w.]+[\w\/]*[\w.]*\??[\w=&\+\%]*/is',$str)){
            return true;
        }
        return false;
    }
