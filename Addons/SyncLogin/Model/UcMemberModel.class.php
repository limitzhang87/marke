 ...
 * 文档基础模型
 */
class UcMemberModel extends Model
{
    /* 用户模型自动完成 */
    ...
    public function registerMember($nickname=''){
        ...
        if($user = $this->create(array('nickname' => $nickname, 'status' => 1, 'email_flag' => 1))){
    ...
    private function autoLogin($user, $remember = false)
    {
        ...
        /*取昵称*/
		$uid=$user['uid'];
		$username=M('uchome_member')->table("zf_uchome_member uch")->join('zf_uc_member uc ON uc.uid=uch.id')->where("uch.id='$uid'")->field('uc.nickname,uch.user_import,uc.user_type,uc.user_flag')->find();
        /* 记录登录SESSION和COOKIES */
        $auth = array(
            'uid' => $user['uid'],
