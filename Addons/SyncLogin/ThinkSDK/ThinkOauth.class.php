...
abstract class ThinkOauth{
    ...
    /*    $config = C("THINK_SDK_{$this->Type}");*/
        ...
        if(empty($config[$type.'_APP_KEY']) || empty($config[$type.'_APP_SECRET'])){
            throw new Exception('请配置您申请的APP_KEY和APP_SECRET');
        } else {

            $this->AppKey    = $config[$type.'_APP_KEY'];
            $this->AppSecret = $config[$type.'_APP_SECRET'];
    ...
    public function config(){
        ...
        if(C('URL_MODEL') == 2){
        	...
        	if($_SERVER[HTTP_HOST]=='user.zfangw.cn'){
        		$this->Callback = "http://$_SERVER[HTTP_HOST]".addons_url('SyncLogin://Base/callback',array('login_user_type'=>1,'type'=>strtolower($this->Type)));
        	}else{
        		$this->Callback = "http://user.zfangw.cn".addons_url('SyncLogin://Base/callback',array('login_user_type'=>1,'type'=>strtolower($this->Type)));
        		 
        	}
        }
        $this->Callback =str_replace("index.php?s=/", "", $this->Callback);
